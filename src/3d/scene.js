 ...
 * 
 * // Debug transparency ordering issues
 * scene.enableDepthLogging(true);
 * ```
 *
...
export default class Scene extends Object3D {
	...
	constructor(name) {
		...
		/**
		 * @type {boolean}
		 * Diagnostic flag: when true, `_depth` will log camera-space Z for each renderable.
		 * Toggle this to true to help debug transparent sorting ordering.
		 */
		this._debugDepthLogs = false;
	...
	/**
	 * Enables or disables depth calculation logging for debugging transparency issues.
	 * When enabled, depth values will be logged to console during rendering.
	 * 
	 * @param {boolean} enabled - Whether to enable depth logging
	 */
	enableDepthLogging(enabled) {
		this._debugDepthLogs = Boolean(enabled);
	}
			...
			if (allBlendRenderables.length <= 1) {
				...
				if (this._sort_transparents) {
					...
					allBlendRenderables.sort((a, b) => {
						...
						// Stable sort tie-breaker using renderable ID for consistent frame-to-frame ordering
	 ...
	 * 
	 * **Debug Logging:**
	 * When `_debugDepthLogs` is enabled, logs depth values to console for troubleshooting
	...
	_depth(renderable) {
		...
		if (typeof this._depthSortStrategy === 'function') {
			const customDepth = this._depthSortStrategy(renderable);
			if (this._debugDepthLogs) {
				try {
					dl(`${renderable.id}: customDepth=${customDepth}`);
		...
		if (this._depthSortStrategy === 'distance') {
			...
			if (this._debugDepthLogs) {
				try {
					dl(`${renderable.id}: distanceSquared=${depthValue}, cameraSpacePos=${cameraSpacePos.x.toFixed(3)},${cameraSpacePos.y.toFixed(3)},${cameraSpacePos.z.toFixed(3)}`);
				} catch (e) { /* ignore console errors */ }
			}
		} else {
			...
			if (this._debugDepthLogs) {
				try {
					dl(`${renderable.id}: cameraZ=${depthValue}`);
				} catch (e) { /* ignore console errors */ }
			}
		}
